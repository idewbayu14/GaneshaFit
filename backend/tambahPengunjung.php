...
if (!empty($_POST['id_pengunjung_pendaftaran'])) {
    ...
    if (!empty($_POST['personal_trainer']) && $_POST['personal_trainer'] === 'gunakan' && !empty($_POST['id_pt'])) {
    ...
    mysqli_stmt_bind_param($stmt, "sssssssss", $id_pengunjung, $nama_lengkap, $no_hp, $alamat, $jenis_pengunjung, $tgl_bergabung, $masa_berlaku, $amount, $id_pt);
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        $stmtKunjungan = mysqli_prepare($conn, $queryKunjungan);
        mysqli_stmt_bind_param($stmtKunjungan, "sss", $id_kunjungan, $id_pengunjung, $id_petugas);

        if (mysqli_stmt_execute($stmtKunjungan)) {
